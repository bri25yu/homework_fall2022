    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            if self.normalize_rnd:
                current_rnd_rew_std = expl_bonus.std()
                expl_bonus = expl_bonus / (self.running_rnd_rew_std + 1e-8)
                self.running_rnd_rew_std = self.rnd_gamma * self.running_rnd_rew_std + (1 - self.rnd_gamma) * current_rnd_rew_std
            ...
            mixed_reward = explore_weight * expl_bonus + exploit_weight * env_reward
            ...
            explotation_critic_v_loss = self.exploitation_critic.update_v(ob_no, ac_na)
            explotation_critic_q_loss = self.exploitation_critic.update_q(ob_no, ac_na, next_ob_no, env_reward,terminal_n)
            ...
            log['Exploitation Critic V Loss'] = explotation_critic_v_loss['Training V Loss']
            log['Exploitation Critic Q Loss'] = explotation_critic_q_loss['Training Q Loss']
