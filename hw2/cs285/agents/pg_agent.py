...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        train_log = self.actor.update(
            observations, actions, advantages, q_vals, len(rewards_list)
        )
    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
        ...
        if self.nn_baseline:
            ...
            values = (values_unnormalized * q_values_std) + q_values_mean
    ...
    def _discounted_cumsum(self, rewards):
        ...
        discounted_cumsum = res.reshape(rewards.shape)
