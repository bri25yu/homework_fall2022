...
from torch.distributions import AffineTransform, ComposeTransform, Normal, TanhTransform
...
class PolicyBase(Module):
    ...
    def __init__(self, environment_info: EnvironmentInfo) -> None:
        ...
        action_range_transform = AffineTransform(loc=action_range_loc, scale=action_range_scale)

        self.action_transform = ComposeTransform([TanhTransform(), action_range_transform])
    ...
    def create_continuous_actions_distribution(self, loc: torch.Tensor, log_scale: torch.Tensor) -> torch.distributions.Distribution:
        ...
        action_dist = self.action_transform(actions_dist_unscaled)
