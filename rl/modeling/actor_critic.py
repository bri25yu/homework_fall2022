...
class ActorCriticBase(PolicyBase):
    ...
    def forward(self, trajectories: Trajectory) -> ModelOutput:
        ...
        V_s_prime = self.critic(trajectories.next_observations).detach()
        Q_s_a = trajectories.rewards + gamma * V_s_prime * (1 - trajectories.terminals)
        advantages = self._normalize(Q_s_a - V_s)

        policy_loss = (-action_log_probs * (advantages.detach())).sum()
        critic_loss = self.critic_loss_fn(V_s, Q_s_a)
